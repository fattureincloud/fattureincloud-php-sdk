...
class IssuedEInvoicesApi
{
     ...
     * @throws \FattureInCloud\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return OneOfVerifyEInvoiceXmlErrorResponseVerifyEInvoiceXmlResponse|\FattureInCloud\Model\VerifyEInvoiceXmlErrorResponse
     */
     ...
     * @throws \FattureInCloud\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of OneOfVerifyEInvoiceXmlErrorResponseVerifyEInvoiceXmlResponse|\FattureInCloud\Model\VerifyEInvoiceXmlErrorResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function verifyEInvoiceXmlWithHttpInfo($company_id, $document_id)
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('OneOfVerifyEInvoiceXmlErrorResponseVerifyEInvoiceXmlResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                    }
                    ...
                    return [
                        ObjectSerializer::deserialize($content, 'OneOfVerifyEInvoiceXmlErrorResponseVerifyEInvoiceXmlResponse', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 422:
            ...
            $returnType = 'OneOfVerifyEInvoiceXmlErrorResponseVerifyEInvoiceXmlResponse';
            if ($returnType === '\SplFileObject') {
                $content = $response->getBody(); //stream goes to serializer
            } else {
                $content = (string) $response->getBody();
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        'OneOfVerifyEInvoiceXmlErrorResponseVerifyEInvoiceXmlResponse',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;

...
class IssuedDocumentsApi
{
    ...
    public function listBinIssuedDocumentsWithHttpInfo($company_id, string $contentType = self::contentTypes['listBinIssuedDocuments'][0])
    {
        ...
        try {
            ...
            switch ($statusCode) {
                case 200:
                    if ('\FattureInCloud\Model\ListBinIssuedDocuments' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\FattureInCloud\Model\ListBinIssuedDocuments' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\FattureInCloud\Model\ListBinIssuedDocuments', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

...
class Entity implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    const DEFAULT_PAYMENT_TERMS_TYPE_STANDARD = 'standard';
    const DEFAULT_PAYMENT_TERMS_TYPE_END_OF_MONTH = 'end_of_month';
     ...
     * @return string[]
     */
    public function getDefaultPaymentTermsTypeAllowableValues()
    {
        return [
            self::DEFAULT_PAYMENT_TERMS_TYPE_STANDARD,
            self::DEFAULT_PAYMENT_TERMS_TYPE_END_OF_MONTH,
        ];
    }
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['default_payment_terms_type'] = $data['default_payment_terms_type'] ?? 'standard';
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getDefaultPaymentTermsTypeAllowableValues();
        if (!is_null($this->container['default_payment_terms_type']) && !in_array($this->container['default_payment_terms_type'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'default_payment_terms_type', must be one of '%s'",
                $this->container['default_payment_terms_type'],
                implode("', '", $allowedValues)
            );
        }
    ...
    {
        $allowedValues = $this->getDefaultPaymentTermsTypeAllowableValues();
        if (!is_null($default_payment_terms_type) && !in_array($default_payment_terms_type, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'default_payment_terms_type', must be one of '%s'",
